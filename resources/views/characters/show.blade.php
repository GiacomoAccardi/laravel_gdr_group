            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="text-center character-stats">
                        ...
                        <a href="{{ route('characters.edit', ['character' => $character['id']]) }}" class="btn btn-warning mt-3 fs-2"><i class="bi bi-feather"></i>Riscrivi</a>

                        <form action="{{ route('characters.destroy', ['character' => $character['id']]) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger mt-3 delete-button fs-2" onclick="return confirm('Sei sicuro di voler cancellare questo personaggio?')">Bandisci</button>
                        </form>
                    </div>
                </div>
            </div>
