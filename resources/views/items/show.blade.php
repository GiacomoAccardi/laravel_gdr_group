            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="text-center stats">
                        ...
                        <a href="{{ route('items.edit', ['item' => $item['id']]) }}" class="btn btn-warning mt-3 fs-2"><i class="bi bi-feather"></i>Riscrivi</a>


                        <form action="{{ route('items.destroy', ['item' => $item['id']]) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger mt-3 delete-button fs-2" onclick="return confirm('Sei sicuro di voler cancellare questo oggetto?')">Distruggi</button>
                        </form>
                    </div>
                </div>
            </div>
